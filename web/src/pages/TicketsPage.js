...
export function TicketsPage({ business }) {
  ...
  }, [loading, error, tickets, itemId, business.id, navigate]);

  const selected = tickets.find((item) => item.id === itemId);

  return (
    <div className="grid h-full min-h-0 lg:grid-cols-[minmax(280px,360px)_minmax(0,1fr)]">
      <section className="min-h-0 overflow-y-auto border-b border-line lg:border-b-0 lg:border-r">
        ...
        {loading ? (
          <p className="px-5 text-sm text-muted">Loading tickets…</p>
        ) : tickets.length === 0 ? (
              ...
              return (
                <li key={ticket.id}>
                  <Link
                    ...
                    className={cn(
                      "block w-full border-t border-line px-5 py-3.5 text-left text-ink no-underline",
